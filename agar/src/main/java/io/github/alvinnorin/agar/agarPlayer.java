...
public class agarPlayer {
    ...
    static HashMap<Player, Integer> mass = new HashMap();
    ...
    public static int fetchSkinThreadId = 0;
    public static void fetchSkin(final Player player) {
    	if (!agars.containsKey(player))
    	fetchSkinThreadId = command.plugin.getServer().getScheduler().runTaskAsynchronously((Plugin)command.plugin, new Runnable(){
            public void run() {
            	command.plugin.getLogger().info("skin");
    ...
    public static void clean(Player player, int[] pos, int[] nev) {
    			...
    			for (int y = -5; y < 4; ++y) {
                	...
                	for (Player online : Bukkit.getOnlinePlayers()) {
                		if (online.getPlayer() != player)
                			if (matrix.contains(online, pos[0] + x-4, pos[2] + y))
                				col = true;
                	}
                	if (!col)
    				for (Player online : Bukkit.getOnlinePlayers()) {
        ...
        for (int y = -4; y < 6; ++y) {
            for (int x2 = -1; x2 < 9; x2 += 9) {
            	...
            	for (Player online : Bukkit.getOnlinePlayers()) {
            		if (online.getPlayer() != player)
            			if (matrix.contains(online, player.getLocation().getBlockX()-4 + x2, player.getLocation().getBlockZ() + y-1))
            				col = true;
            	}
            	if (!col)
            	for (Player online : Bukkit.getOnlinePlayers()) {
        ...
        for (int x = 0; x < 8; ++x) {
            for (int y2 = -5; y2 < 4; y2 += 9) {
            	...
            	for (Player online : Bukkit.getOnlinePlayers()) {
            		if (online.getPlayer() != player)
                		if (matrix.contains(online, player.getLocation().getBlockX()-4 + x, player.getLocation().getBlockZ() + y2))
            				col1 = true;
            	}
            	for (Player online : Bukkit.getOnlinePlayers()) {
            		if (online.getPlayer() != player)
            			if (matrix.contains(online, player.getLocation().getBlockX()-4 + x, player.getLocation().getBlockZ()+4))
            				col2 = true;
            	}
            	if (!col1 || !col2)
            	for (Player online : Bukkit.getOnlinePlayers()) {
    ...
    static HashMap<Player, Boolean> agars_thread = new HashMap();
    public static byte getSkin(Player player, int x, int y) {
        ...
        try {
        	...
        	try {
	        	if (!agars_thread.get(player)) {
		        	//if (!command.plugin.getServer().getScheduler().isCurrentlyRunning(fetchSkinThreadId))
		        	agarPlayer.fetchSkin(player);
		        	agars_thread.put(player, true);
    ...
    public static void spawn(Player player, int color, int[] pos) {
    	boolean hasEaten = food.hasEaten(player);
    	//try {mass.get(player);} catch (NullPointerException e) {}
    	if (hasEaten) {
    		mass.put(player, mass.get(player) + 1);
    		player.sendMessage(mass.get(player)+"");
    	}
    	matrix.clear(player);
        for (int y = 0; y < 8; ++y) {
            for (int x = 0; x < 8; ++x) {
